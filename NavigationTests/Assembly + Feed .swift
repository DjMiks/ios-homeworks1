...
class FeedViewControllerForTest: UIViewController {
    ...
    private lazy var button1 = CustomButton(color: .orange, title: "Show My Post", titleColor: .white) {
        self.showPost()
    }
    ...
    private func checkPass() {
        do {
            ...
            let ac = UIAlertController(title: "Пусто", message: "Пустое поле", preferredStyle: .alert)
            let OKButton = UIAlertAction(title: "ОК", style: .default)
            ac.addAction(OKButton)
            self.present(ac, animated: true)
        } catch {
            let ac = UIAlertController(title: "Мимо!", message: "Вы не угадал!", preferredStyle: .alert)
            let OKButton = UIAlertAction(title: "Попытайтесь снова", style: .default)
            ac.addAction(OKButton)
            self.present(ac, animated: true)
